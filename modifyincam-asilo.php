...
// log the attempt
	...
	if( !isset($_SESSION['myusername']) ) {
		...
		$host="89.46.111.32"; // Host name 
		$username="Sql1036019"; // Mysql username 
		$password="3puek72gci"; // Mysql password 
		$db_name="Sql1036019_1"; // Database name 
...
//Connect to the server and select the database
      ...
      <?php
		$sql6 = "SELECT money.*, cane.name as cane, padr.family||' '||padr.name as padrone FROM `uln6tjuc_libdog`.`libdog_prendi_e_scappa` as money, `uln6tjuc_libdog`.`libdog_Anagrafica_Cani` as cane, `uln6tjuc_libdog`.`libdog_Anagrafica_Clienti` as padr WHERE  `id`='$id_money' and id_cane = cane.idlibdog_Anagrafica_Cani and id_cliente = padr.idlibdog_Anagrafica_Clienti;";
			$resultentry = mysql_query($sql6);
			while($row = mysql_fetch_array($resultentry)){
			?>
      <div class = "container">
        	<div id='loginform'>
            <form action="modifyincam.php?id=<?php echo $id_money ?>" method='post' id='login'>
			  <table>
			  <tr> <td> <font color=#eeeeee> Giorno: </font>   </td> 
				<td> <font color=#eeeeee> ID cliente: </font> </td>
				<td><font color=#eeee00> ID cane: </font></td>  
			  </tr> 
			  <tr> <td bgcolor=#aaaa00> <input type='date' name='data_trans' value="<?php echo $row['data_trans']; ?>"/></td> 
				<td bgcolor=#aaaa00> <input type='text' name='id_cliente' value="<?php echo $row['id_cliente']; ?>"/> </td> 
				<td> <input type='time' name='id_cane' value="<?php echo $row['id_cane']; ?>"/></td>
              </tr>
			  ...
			  <tr> <td><input type='text' name='importo_fattura' value="<?php echo $row['importo_fattura']; ?>"/>  </td>
			    <td> <input type='time' name='n_fattura' value="<?php echo $row['n_fattura']; ?>"/> </td>
			  ...
			  <tr> <td><input type='text' name='commenti' value="<?php echo $row['commenti']; ?>"/>  </td>
			    <td> <input type='time' name='tipologia' value="<?php echo $row['tipologia']; ?>"/> </td>
			  ...
			  <tr> <td><input type='text' name='privato' value="<?php echo $row['privato']; ?>"/>  </td>
			    <td> <input type='time' name='tipo_pagamento' value="<?php echo $row['tipo_pagamento']; ?>"/> </td>
			  ...
			  <tr> <td><input type='text' name='quantita' value="<?php echo $row['quantita']; ?>"/>  </td>
			    <td> <input type='time' name='cane' value="<?php echo $row['cane']; ?>"/> </td>
				<td> <input type='text' name='padrone' value="<?php echo $row['padrone']; ?>"/> </td>
			  </tr>
			  <tr>  
					<td> </td> 
					<td> </td> 
