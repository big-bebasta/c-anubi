...
// To protect MySQL injection (more detail about MySQL injection)
                  ...
                  $sql2 = "INSERT INTO `uln6tjuc_libdog`.`libdog_presenze_day` (`giorno`,`id_cane`, `id_pagamento`, `intime`, `outtime`, `note`) VALUES ('$giorno', '$id_cane','$id_pagamento', '$intime', '$outtime','$note');";
      ...
      <div class = "container">
            ...
            <form action="dogin.php" method='post' id='login'>
			  ...
			  <tr> <td> <input type='date' name='giorno' /></td> 
				<td> 
         <?php
				$sqlcliente = "SELECT CONCAT(`idlibdog_Anagrafica_Cani`,'-',`name`,'-',`breed`) as dog FROM `libdog_Anagrafica_Cani`;";
         ...
         ?>				
				...
				<td> 
		<?php
				$sqlpaga = "SELECT CONCAT(`id`,'-',`descrizione`,'-',`tipologia`) as paga FROM `libdog_tariffario_all`;";
...
<?php
		...
		foreach($row_quanti as $cell_quanti)
			if ($counterquanti < 1) {
				echo "<font align=center><h2>$cell_quanti</h2></font>";
        		$counterquanti++;
				}
		mysql_query($queryquanti);
		?>
		cani, uno pi&ugrave; pirla dell' altro.
		</h2>
					<table align=center border=2> <tr><td>Giorno</td><td>Nome Cane</td><td>Razza</td><td>Tipologia</td><td>Entrata</td><td>Uscita</td></tr>
						<?php 
