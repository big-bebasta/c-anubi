...
// To protect MySQL injection (more detail about MySQL injection)
                  ...
                  $sql2 = "INSERT INTO `uln6tjuc_libdog`.`libdog_prendi_e_scappa` (`data_trans`, `id_cliente`,`importo_fattura`, `n_fattura`, `descrizione`, `commenti`, `tipologia`, `note`, `privato`,`tipo_pagamento`,`ric_fat`,`quantita`) VALUES ('$data_trans', '$id_cliente','$importo_fattura', '$n_fattura', '$descrizione', '$commenti', '$tipologia','$note','$privato','$tipo_pagamento','$ric_fat','$quantita');";
      ...
      <div class = "container">
            ...
            <form action="incam.php" method='post' id='login'>
			  ...
			  <tr> <td> <input type='date' name='data_trans' /></td> 
				<td> 
         <?php
				$sqlcliente = "SELECT CONCAT(`idlibdog_Anagrafica_Clienti`,'-',`libdog_Anagrafica_Clienti`.`name`,'-',`libdog_Anagrafica_Clienti`.`family`,'-',`libdog_Anagrafica_Cani`.`name`) as clio FROM `libdog_Anagrafica_Clienti`, `libdog_Anagrafica_Cani` where `libdog_Anagrafica_Clienti`.`idlibdog_Anagrafica_Clienti`=`libdog_Anagrafica_Cani`.`libdog_Anagrafica_Clienti_id` order by `libdog_Anagrafica_Cani`.`name`;";
         ...
         ?>				
			  ...
			  <tr> <td> <input type='text' name='n_fattura' /> </td>
				<td> 
		<?php
				$sqldesc = "SELECT CONCAT(`id`,'-',`descrizione`,'-',`tipologia`) as descr FROM `libdog_tariffario_all` order by tipologia, id;";
         ...
         ?>	
			  ...
			  <tr> <td> <input type='number' name='quantita' /> </td>
				...
				<td> <select name='ric_fat'>
						<option value='Contanti'>Ricevuta</option>
						<option value='Bancomat'>Fattura</option>
						</select> </td>
			  </tr>
...
if ($userkind == 'teamleader') {
				...
				echo "	<table align=center border=2> <tr><td>Identificativo</td><td>Data</td><td>Nome Cane</td><td>Razza Cane</td><td>Cognome Padrone</td><td>Importo</td><td>N. fattura</td><td>Descrizione</td><td>Commenti</td><td>Tipologia</td><td>Tipo pagamento</td><td>Note</td><td>Privato</td></tr>";

	
						$sql = "SELECT `id`,`data_trans`, `libdog_Anagrafica_Cani`.`name`, `libdog_Anagrafica_Cani`.`breed`,`libdog_Anagrafica_Clienti`.`family`,`importo_fattura`, `n_fattura`, `descrizione`, `commenti`, `tipologia`, `tipo_pagamento`,`note`, `privato`  FROM `libdog_prendi_e_scappa` , `libdog_Anagrafica_Cani`, `libdog_Anagrafica_Clienti`  where `libdog_Anagrafica_Clienti_id` = `id_cliente` and `libdog_Anagrafica_Clienti`.`idlibdog_Anagrafica_Clienti` = `id_cliente` and `data_trans` > CURDATE() - 120 order by `data_trans` desc;";
...
if ($userkind == 'superuser') {
				...
				echo "	<table align=center border=2> <tr><td>Identificativo</td><td>Data</td><td>Nome Cane</td><td>Razza Cane</td><td>Cognome Padrone</td><td>Importo</td><td>N. fattura</td><td>Descrizione</td><td>Commenti</td><td>Tipologia</td><td>Tipo pagamento</td><td>Note</td><td>Privato</td></tr>";
						...
						$sql = "SELECT `id`,`data_trans`, `libdog_Anagrafica_Cani`.`name`, `libdog_Anagrafica_Cani`.`breed`,`libdog_Anagrafica_Clienti`.`family`,`importo_fattura`, `n_fattura`, `descrizione`, `commenti`, `tipologia`, `tipo_pagamento`,`note`, `privato`  FROM `libdog_prendi_e_scappa` , `libdog_Anagrafica_Cani`, `libdog_Anagrafica_Clienti`  where `libdog_Anagrafica_Clienti_id` = `id_cliente` and `libdog_Anagrafica_Clienti`.`idlibdog_Anagrafica_Clienti` = `id_cliente` order by `data_trans` desc;";
						$result = mysql_query($sql);
						$counter=0;
						while($row = mysql_fetch_row($result)) {
					    echo "<tr>";
    ...
    // of $row to $cell variable					
				...
				echo "	<table align=center border=2> <tr><td>Totale</td><td>Privato</td><td>Mese</td><td>Tipologia</td><td>Cliente</td></tr>";
...
//		SELECT sum(`importo_fattura`) as totale, `privato`, DATE_FORMAT(`data_trans`,'%Y-%m') as mese, `tipologia`,`id_cliente` FROM `libdog_prendi_e_scappa` group by `privato`, DATE_FORMAT(`data_trans`,'%Y-%m'), `tipologia`, `id_cliente` ;
		...
		$sqlrecap = "		SELECT sum(`importo_fattura`) as totale, `privato`, DATE_FORMAT(`data_trans`,'%Y-%m') as mese, `tipologia`, `id_cliente`  FROM `libdog_prendi_e_scappa` group by `privato`, DATE_FORMAT(`data_trans`,'%Y-%m'), `tipologia`, `id_cliente` order by DATE_FORMAT(`data_trans`,'%Y-%m');";
