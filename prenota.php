...
//						$startDate = date_format($date,"Y-m-d")."T".date_format($date,"H:i:s")."+01:00";
      ...
      <div class = "container">
        	<div id='loginform'>
            <form action="prenota.php" method='post' id='login'>
              <table>
			  ...
			  <tr> <td> <input type='date' name='giorno' /></td> 
				<td> 
         <?php
				$sqlcliente = "SELECT CONCAT(`idlibdog_Anagrafica_Cani`,'-',`name`,'-',`breed`) as dog FROM `libdog_Anagrafica_Cani` where libdog_Anagrafica_Clienti_id=$client_id order by `name`;";
		...
		// 		aggiunto id orainizio
         ...
         ?>				
				...
				<td> 
		<?php
				$sqlpaga = "SELECT distinct CONCAT(cosa.`id`,'-',cosa.`descrizione`,'-',cosa.`tipologia`) as paga FROM `libdog_tariffario_all` as cosa , `libdog_prendi_e_scappa` as money where cosa.`tipologia` = 'Asilo' and ( money.`descrizione`= CONCAT(cosa.`id`,'-',cosa.`descrizione`,'-',cosa.`tipologia`) or  money.`descrizione` = CONCAT(cosa.`id`,'-',cosa.`descrizione`,'-',cosa.`tipologia`,'-',cosa.`carnet`)) and money.id_cliente = $client_id;";
		...
		// aggiunto id orafine e tolto value che in un select non serve a nulla :)
         ...
         ?>	
			  ...
			  <td><input data-toggle="popover" title="Vincoli di prenotazione" data-content="La prenotazione con taxi deve avvenire almeno 36 ore prima del momento di prelievo o consegna del cane richiesti." type='submit' name = "Inserisci" value='Inserisci' /></td> 
			  <td></td> </tr>
			  </table>
            </form>
        	</div>
...
<?php
	if ($printoutmessage==1) { echo "<h2 align=center> Prenotazione inserita</h1>"; $printoutmessage=0;}  			
?>
