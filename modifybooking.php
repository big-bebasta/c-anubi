...
// log the attempt
	...
	if( !isset($_SESSION['myusername']) ) {
		...
		$id_day = $_GET['id_day'];	
		$getcalendaridfromdb = mysqli_fetch_assoc(mysqli_query($link, "SELECT calendar FROM libdog_presenze_day WHERE id_day = '$id_day'")); 
...
// To protect MySQL injection (more detail about MySQL injection)
				...
				//Put here the authentication part of the code
                  ...
                  $sql1 = "UPDATE `uln6tjuc_libdog`.`libdog_presenze_day` SET `intime`='$intime',`outtime`='$outtime', `note`='$note', `trasporto`='$trasporto' WHERE `id_day`='$id_day';";
				//  echo ( $sql1 );
      ...
      <div class = "container">
        	<div id='loginform'>
            <form action="modifybooking.php?id_day=<?php echo $id_day ?>" method='post' id='login'>
			  <table>
...
<?php 
 if ($userkind == 'superuser') {
	echo "<h4><a href=\"booking.php\">Return to Prenotazioni</a></h4> <br> <h6>";
 } else {
	 if ($userkind == 'user') {
	echo "<h4><a href=\"booking.php\">Return to Prenotazioni</a></h4> <br><br><br><br><h6>";
			}
		}	
			echo ( $msgupdateentry );
		?>
