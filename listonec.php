...
<br><br><br>	
					...
					<table align=center border=2> <tr> <td align=center> Tipologia </td> <td align=center> Descrizione </td> <td align=center> Prezzo </td> </tr>
						<?php 
...
//Connect to the server and select the database
						...
						$sql = "SELECT  `id` , `tipologia` ,`denominazione`, `descrizione` ,  `importo`  FROM  `libdog_tariffario_all` order by tipologia, denominazione,descrizione;";
    ...
    // of $row to $cell variable					
						    foreach($row as $cell)
        					if ($counter < 5) {
									if ($counter == 0) { $dogid=$cell;  }
									if ($counter == 1) {  echo "<td><h3>$cell</h3></td>"; }
									if ($counter == 2) { if ($cell=="") { echo "&nbsp;"; $switch=1;} else { echo "<td><h3>$cell</h3></td>"; } }
									if ($counter == 3) { if ($switch==1) { echo "<td><h3>$cell</h3></td>";  $switch=0; } else { echo "&nbsp;"; } }
									if ($counter == 4) {  echo "<td><h3>$cell</h3></td>"; }
        							$counter++;
        					} else { 
        							echo ""; 
        							}
