...
//	$vout=date('d-m-Y',$out[$i]);
	...
	echo "\">";
		...
		{
			$back=($statusarray[$i]==0)?$prenotato['1']:$confermato['1']; // controllo se la prenotazione è confermata o meno
			...
			echo "<a href=\"prenotazioni.php?id=".$id_book[$i]."\" title=\"Entrato alle ".$intimearray[$i]." uscito alle ".$outtimearray[$i]." Note: ".$notearray[$i]." \"><img src=\"images/".$back."\" alt=\"\" /></a>";
		}
		else if ($team[$i]=='2')  // se il valore è uguale a 2 e quindi la team è occupata tutta la giornata
		{
			$back=($statusarray[$i]==0)?$prenotato['2']:$confermato['2'];
			echo "<a href=\"prenotazioni.php?id=".$id_book[$i]."\" title=\"Entrato alle ".$intimearray[$i]." uscito alle ".$outtimearray[$i]." Note: ".$notearray[$i]." \"><img src=\"images/".$back."\" alt=\"\" /></a>";
		}
		else if ($team[$i]=='3') // se il valore è uguale a 3 e quindi abbiamo una fine prenotazione ed un inizio prenotazione lo stesso giorno
		{
			$back=($statusarray[$i]==0)?$prenotato['3']:$confermato['3'];
			echo "<a href=\"prenotazioni.php?id=".$id_book[$i]."\" title=\"Entrato alle ".$intimearray[$i]." uscito alle ".$outtimearray[$i]." Note: ".$notearray[$i]." \"><img src=\"images/".$back."\" alt=\"\" /></a>";
		}
...
if ( $printdate==10 ) {
	...
	for ($i=1; $i<=$dati['giorni_mese'];$i++) {
		if ($d3=="7") $d3=0;
	?>	
		<td style="text-align:center;<? if ($d3=="6" || $d3=="0") echo "background-color:red;"; ?>"><? echo $i; ?></td>
	<?	
		$d3++;
		}
	echo "</tr>";
